...
def orchestrator_function(context: df.DurableOrchestrationContext):
    ...
    # user dag execution
    kskn = yield context.call_activity("Splitter", serwoObject)

    serwo_to_dict = unmarshall(json.loads(kskn))
    body = serwo_to_dict.get_body()
    obj = body["List"]
    std_meta = serwo_to_dict.get_metadata()
    zsiz = []
    xvdz = []

    for i in range(0, len(obj)):
        var = {"body": obj[i], "metadata": std_meta}
        var = build_serwo_object(var).to_json()
        zsiz.append(context.call_activity("Transpiler", var))
        xvdz.append(context.call_activity("Submitter", zsiz[i]))

    xvdz = yield context.task_all(xvdz)
    body["List"+"Results"] = serwolist_to_obj(xvdz)
    var2 = {"body": body, "metadata": std_meta}
    xvdz = build_serwo_object(var2).to_json()
    qmax = yield context.call_activity("Merger", xvdz)
    zzio = yield context.call_activity("Poller", qmax)
    ygtl = yield context.call_activity("Reconstructor", zzio)
    ygtl = insert_end_stats_in_metadata(ygtl)
    iqyr = yield context.call_activity("CollectLogs", ygtl)
    return iqyr
